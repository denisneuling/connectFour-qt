...
GameBoard::GameBoard(int rows, int columns, QObject *parent) :
    QObject(parent), ConnectFourObject(columns, rows)
{
    for (int i = 0; i < m_Columns; ++i)
        m_pGameBoard.push_back(std::make_shared<BoardColumn>(m_Rows));
}

GameBoard::GameBoard(const GameBoard &other) : QObject(other.parent()), ConnectFourObject(other.m_Columns, other.m_Rows)
{
